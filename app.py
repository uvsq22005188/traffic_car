...
class Ville():
    ...
    def creer_menu(self):
        ...
        for i in range(1, 5):
            self.menu_Route.add_command(image=self.image['route'][i], command=lambda: print(f"route {i}"))

        # menu tournant
        self.menu_Tourant = tk.Menu(self.menu_bar, tearoff=0)
        self.menu_Création.add_cascade(label="Tourant", menu=self.menu_Tourant)
        for i in range(1, 5):
            self.menu_Tourant.add_command(image=self.image['tournant'][i], command=lambda: print(f"tournant {i}"))
        ...
        for i in range(1, 5):
            self.menu_RP1.add_command(image=self.image['rp'][1][i], command=lambda: print(f"RP 1 - {i}"))
